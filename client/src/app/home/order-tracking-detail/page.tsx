...
const statusIcon = (status: string) => {
  switch (status) {
    case 'pending':
      return (
        <MdOutlinePending className='text-yellow-500' title='Chờ xác nhận' />
      );
    case 'ready_for_pickup':
    case 'out_for_delivery':
      return (
        <MdOutlineLocalShipping className='text-blue-500' title='Đang giao' />
      );
    case 'delivered':
      return (
        <MdOutlineCheckCircle className='text-green-600' title='Đã giao' />
      );
    case 'cancelled':
    case 'returned':
      return (
        <MdOutlineCancel className='text-red-500' title='Đã huỷ/Trả hàng' />
      );
    default:
          ...
          <table className='w-full text-sm bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200'>
            ...
            <tbody className='divide-y divide-gray-100'>
              {userOrders.map((order: any) => (
                ...
                >
                  <td className='p-4 font-semibold text-[#C4265B]'>
                    {order.orderNumber || order._id}
                  </td>
                  <td className='p-4'>
                    {new Date(order.createdAt).toLocaleString()}
                  </td>
                  <td className='p-4 text-right text-[#C4265B] font-bold'>
                  ...
                  <td className='p-4'>
                    <button
                      ...
                      onClick={() =>
                        navigate(`/home/order-tracking/${order._id}`)
                      }
                    >
                      Chi tiết
                    </button>
