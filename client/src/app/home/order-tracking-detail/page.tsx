    ...
    //                     <div key={order._id} className="border-black-100 rounded-lg shadow-sm p-4 bg-white">
    ...
    //                                         {order.shop?.fullName || 'Cửa hàng'} ({order.shop?.username || 'username'})
    //                                     </p>
    //                                 </div>
    //                             </div>
    ...
    //                                 <span className={`ml-2 px-2 py-0.5 text-sm font-semibold rounded-full ${statusStyle(order.status)}`}>
    //                                     {translateStatus(order.status)}
    //                                 </span>
    //                             </p>
    //                             <p className="flex items-center gap-2">
    //                                 <FaMoneyBillWave className="text-green-500" />
    //                                 <strong>Phương thức thanh toán:</strong> {order.payment?.method?.toUpperCase() || '---'} - {order.payment?.status || '---'}
    //                             </p>
    //                             <p className="flex items-center gap-2">
...
import { useDispatch, useSelector } from 'react-redux';
import { filterOrders, updateOrder } from '~/store/slices/orderSlice';
import type { RootState } from '~/store';
import { FaStore, FaCalendarAlt, FaMoneyBillWave, FaShippingFast, FaClipboardCheck, FaExclamationCircle } from 'react-icons/fa';
import { fetchVariants } from '~/store/slices/variantSlice';
import { AuthContext } from '~/contexts/authContext';
                ...
                {orders.map((order: any) => (
                    <div key={order._id} className="border-black-100 rounded-lg shadow-sm p-4 bg-white">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button className="bg-pink-100 text-[#C4265B] px-3 py-1 rounded text-sm font-medium hover:bg-pink-200">
                                <Link to={`/home/shop-profile/${order.shop?._id}`}>Xem shop</Link>
                            </button>
                        </div>
