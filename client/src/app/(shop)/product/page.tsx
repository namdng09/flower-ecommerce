...
const Page = () => {
  ...
  // Lấy shopId từ currentUser
  const shopId = currentUser?.role === 'shop' ? String(currentUser._id) : null;
  ...
  const confirmDelete = () => {
    if (selectedId) {
      // TODO: dispatch action xóa sản phẩm
      setSelectedId(null);
    }
  };
        ...
        <div className='space-y-1'>
          {value?.slice(0, 2).map((variant, i) => (
            <div key={i} className='text-xs text-gray-600'>
              {variant.title} - <b>{variant.salePrice.toLocaleString()}đ</b> (
