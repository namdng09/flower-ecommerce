...
const ReviewPage: React.FC = () => {
    ...
    const renderProductInfo = () => {
        if (productLoading) return <div className="animate-pulse bg-gray-200 h-48 rounded mb-8"></div>;
        if (productError) return <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-8">{productError}</div>;
                    ...
                    <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div className="flex items-center">
                                <span className="font-semibold text-gray-700 mr-2">Giá từ:</span>
                                <span className="text-2xl font-bold text-red-600">{minPrice?.toLocaleString()}₫</span>
                            </div>
                            <div className="flex items-center">
                                <span className="font-semibold text-gray-700 mr-2">Shop:</span>
                                <span className="text-gray-800">{product.shop?.fullName || product.shop?.username}</span>
                            </div>
                        </div>
    ...
    const renderStars = (
        ...
        const stars = [1, 2, 3, 4, 5].map((starNumber) => {
            ...
            const interactiveClass = interactive ? 'hover:text-yellow-400 cursor-pointer' : '';
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
                ...
                {user ? (
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                        <p className="text-yellow-800">Vui lòng đăng nhập để viết đánh giá sản phẩm.</p>
                    </div>
                )}
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div>
                                                <div className="font-semibold text-gray-900">
                                                    {review.userId?.fullName} ({review.userId?.username} - {review.userId?.email})
                                                </div>
